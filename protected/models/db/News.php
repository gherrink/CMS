...
class News extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('newsid, label, roleaccess, update_userid, update_time, create_userid', 'required'),
			array('newsid', 'length', 'max'=>32),
			array('label, update_userid, create_userid', 'length', 'max'=>20),
			array('roleaccess', 'length', 'max'=>64),
			array('create_time', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('newsid, label, roleaccess, update_userid, update_time, create_userid, create_time', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'languages' => array(self::MANY_MANY, 'Language', 'NewsLanguage(newsid, languageid)'),
		);
	}
