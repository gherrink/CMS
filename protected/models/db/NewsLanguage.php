...
/**
 * This is the model class for table "NewsLanguage".
 *
 * The followings are the available columns in table 'NewsLanguage':
...
class NewsLanguage extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('newsid, languageid, text', 'required'),
			array('newsid', 'length', 'max'=>32),
			array('languageid', 'length', 'max'=>2),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('newsid, languageid, text', 'safe', 'on'=>'search'),
		);
	}
