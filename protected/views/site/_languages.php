...
if(isset($_POST['SiteLanguage']))
{
	foreach ($_POST['SiteLanguage'] as $attributs)
	{
		...
		$this->renderPartial('_language', array('model'=>$siteLanguage, 'form'=>$form, 'counter'=>$counter++));
	}
}
else if($model->siteid !== null && $model->siteid !== "")
{
	$siteLanguages = SiteLanguage::model()->findAllByAttributes(array('siteid'=>$model->siteid));
