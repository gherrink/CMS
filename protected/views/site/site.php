        ...
        <div class="col-sm-12 btn-group">
            ...
            if ($editable)
                echo BsHtml::linkButton(MsgPicker::msg()->getMessage(MSG::BTN_EDIT), array(
                    'url' => Yii::app()->createAbsoluteUrl('site/edit', array(
                        'name' => $model->label))
                ));
            else
                echo BsHtml::linkButton(MsgPicker::msg()->getMessage(MSG::BTN_READ), array(
                    'url' => Yii::app()->createAbsoluteUrl('site/read', array(
                        'name' => $model->label))
                ));

            echo BsHtml::button(MsgPicker::msg()->getMessage(MSG::BTN_UPDATE), array(
                'onclick' => 'cmsShowModalAjax("modal", "' . Yii::app()->createAbsoluteUrl('site/update', array(
                    'name' => $model->label)) . '");',
            ));

            $urlDelete = Yii::app()->createAbsoluteUrl('site/delete', array(
                'name' => $model->label));
            ...
            $this->widget('application.widgets.language.LanguageSelector', array(
                'translate' => true, 'languageid' => 'editLng', 'selectedLanguage' => $editLng));
            ?>
        </div>
    </div>
    <hr>
...
$language = SiteLanguage::model()->findByAttributes(array('siteid' => $model->siteid,
    'languageid' => $lng));
if ($language !== null)
...
<?php $this->renderPartial('_' . $model->layout, array('site' => $model,
    'edit' => $edit, 'editLng' => $editLng)) ?>

<?php if ($edit || $editable): ?>
    <hr>
    <div class="row">
        <div class="col-sm-6">
            <p><?php echo MsgPicker::msg()->getMessage(MSG::CREATE_USER_TIME, array(
        'user' => $model->create_userid, 'time' => $model->create_time)) ?></p>
        </div>
        <div class="col-sm-6 text-right">
            <p><?php echo MsgPicker::msg()->getMessage(MSG::UPDATE_USER_TIME, array(
        'user' => $model->update_userid, 'time' => $model->update_time)) ?></p>
        </div>
    </div>
<?php endif; ?>
<?php
